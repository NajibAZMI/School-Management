...
<head>
    ...
    <link rel="stylesheet" href="../../style/Coordinateur/Definiremploi_nv.css">
...
<body>
        ...
        <article class="article1">
          <section class="partieprofesseur">
            <div class="choseplace">
               <div class="classroom"><p>classe emplois</p></div>
            </div>
            <div class="choseresult">
              <?php
  

        $sql = "SELECT nom_classe, id_classe
                FROM classe
                WHERE id_filiere = $selected_filiere_id
               ";
       $result = mysqli_query($conn, $sql);
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<button type="submit" class="button-modules" name="profesr_module"> <p>' . $row['nom_classe'] . '</p> </button>'; 
        echo '<input type="hidden" name="selected_prof_id" value="' . $row['id_classe'] . '">'; 
        echo '</div>';
        echo '</form>';
    }
    ...
    if (isset($_POST['profesr_module'])) {
      ...
      $selected_class_id = $_POST['selected_prof_id'];
      $_SESSION["elected_class_id"] = $selected_class_id ;
...
<div class="palcemodule">
  ...
  <script>
    ...
    function insertValues() {
      ...
      xhr.open('POST', 'emploiclasse.php', true);
